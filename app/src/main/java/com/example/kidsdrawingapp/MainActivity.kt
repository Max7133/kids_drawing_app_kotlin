...
class MainActivity : AppCompatActivity() {
    ...
    private inner class BitmapAsyncTask(val mBitmap: Bitmap) : AsyncTask<Any, Void, String>() {
        ...
        override fun onPostExecute(result: String?) {
            ...
            //For file sharing(it's build in)
            //this@MainActivity and not just "this", because I'm not in the context of MainActivity
            //and I'm inside Async task class
            MediaScannerConnection.scanFile(this@MainActivity, arrayOf(result), null) {
                // to open something up I need a Intent
                // path, uri are parameters, and what is executed
                    path, uri ->
                val shareIntent = Intent()
                shareIntent.action = Intent.ACTION_SEND
                //I have a content uri holding a stream of data associated with the intent, used to supply the data being sent
                shareIntent.putExtra(Intent.EXTRA_STREAM, uri)
                //I give the type(this type here is the mimetype being handled by this intent
                shareIntent.type = "image/png"
                startActivity(
                    // target intent is shareIntent(that I did), title
